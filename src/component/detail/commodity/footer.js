...
export default class Footer extends React.Component{
	...
	handleFooterReduce(index){
		/*减少数量*/
		this.props.handleFooterReduce(index,this.props.mainFoods[index].id)
	}
	_transitionEndCover(){
	...
	render(){
		let list=this.props.mainFoods.map((item,index)=>{
			return(
				<li className='footer_all_tip_main_list' key={index}>
					<span className="entityList-entityname_1">
						<em className="entityList-entityname_1_1">{item.name}</em>
					</span>
					<span className="entityList-entityname_2">
						<span className="entityList-entityname_2_1">{item.price*item.quantity}</span>
					</span>
					<span className="entityList-entityname_3 food_add">
						<Selected  
						quantity={item.quantity} 
						handleReduceAdd={this.handleFooterAdd.bind(this,index)}
						handleReduceFood={this.handleFooterReduce.bind(this,index)}
						/>
					</span>
				</li>
			)
			...
			<footer className='shop_footer' >
				...
				<div className="shop_footer_nav">
					...
					<a href="javascript:void(0)" className={`shoping_submit 
						${this.props.allPrice>=this.props.data.float_minimum_order_amount
						?'':'isdisable'}`}>
						{this.props.allPrice>=this.props.data.float_minimum_order_amount
						?<span>去结算</span>
						:<span>还差¥{this.props.data.float_minimum_order_amount-this.props.allPrice}起送</span>}
					</a>
				</div>
			</footer>
		)
