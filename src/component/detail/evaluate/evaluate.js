...
export default class Evaluate extends React.Component{
	...
	componentDidMount(){
		...
		//this.addEvent(window,'scroll',this._scrollTogether.bind(this));//页面多个window.onscroll共存问题
	}
  ...
  addEvent(obj,type,fn){
	...
	_scrollTogether(){
		/*联动*/
		let headerHeight=document.querySelector('.detail_header').offsetHeight;
	...
	*/
	...
	_getData(name,page,fun){
		fetch(`/ugc/v2/restaurants/${this.props.id}/ratings?has_content=true&tag_name=${name}&offset=${page}0&limit=10`)
